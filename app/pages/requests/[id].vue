<template>
  ...
  <div v-else-if="requestError" class="page-wrap"><StatePanel kind="error" title="Не удалось загрузить заявку" :description="requestError.statusMessage || 'Попробуйте повторить запрос.'" action="Повторить" @action="refresh" /></div>
...
import type { RequestComment } from '#shared/types/domain'
...
const request = computed(() => rawRequest.value ? normalizeRequest(rawRequest.value) : null)
...
const details = computed(() => [
  { label: 'Клиент', value: request.value?.customer ?? '—' }, { label: 'Компания', value: request.value?.customerCompany ?? '—' }, { label: 'Категория', value: request.value?.category ?? '—' }, { label: 'Исполнитель', value: request.value?.assignee ?? 'Не назначен' }, { label: 'Срок SLA', value: request.value ? formatDate(request.value.slaDueAt) : '—' },
])
...
function addComment() { if (!commentBody.value || !request.value) return; comments.value.push({ id: crypto.randomUUID(), requestId: request.value.id, authorId: 'user-operator', author: 'Анна Морозова', avatar: 'АМ', body: commentBody.value, createdAt: new Date().toISOString() }); commentBody.value = '' }
</script>
