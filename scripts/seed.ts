...
try {
  await sql.begin(async transaction => {
    ...
    for (const request of demoRequests) {
      await transaction`INSERT INTO requests (id, title, description, status, priority, customer_id, assignee_id, category_id, sla_due_at, created_at, updated_at, closed_at, archived) VALUES (${request.id}, ${request.title}, ${request.description}, ${request.status}, ${request.priority}, ${request.customerId}, ${request.assigneeId}, ${request.categoryId}, ${request.slaDueAt}, ${request.createdAt}, ${request.updatedAt}, ${request.closedAt}, ${request.archived}) ON CONFLICT (id) DO UPDATE SET title = excluded.title, description = excluded.description, status = excluded.status, priority = excluded.priority, customer_id = excluded.customer_id, assignee_id = excluded.assignee_id, category_id = excluded.category_id, sla_due_at = excluded.sla_due_at, updated_at = excluded.updated_at, closed_at = excluded.closed_at, archived = excluded.archived`
